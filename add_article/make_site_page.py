...
# 'mp': 'meru-para'
star_category = ['コスト', '使いやすさ', '出会える度', 'ピュア', 'アダルト']
...
def main(pd):
    csv_path = 'reibun/csv_data/rb_csv.csv'
...
def insert_mod_log_to_top_page(date_str, pd):
    ...
    with open('reibun/index.html', 'w', encoding='utf-8') as g:
        g.write(long_str)
    with open('reibun/html_files/amp/index.html', 'r', encoding='utf-8') as f:
        long_str_a = f.read()
    ...
    long_str_a = re.sub(r'<div id="update"><ul class="updli">', '<div id="update"><ul class="updli">' + insert_a,
                        long_str_a)
    with open('reibun/html_files/amp/index.html', 'w', encoding='utf-8') as g:
        g.write(long_str_a)
    file_upload.ftp_upload(['reibun/index.html', 'reibun/html_files/amp/index.html'], pd)
...
def insert_data_to_amp_site_page():
    ...
    for pc_path in path_list:
        with open('reibun/html_files/pc/sitepage/' + pc_path, 'r', encoding='utf-8') as f:
            pc_str = f.read()
        ...
        print(base_str)
        with open('reibun/html_files/amp/sitepage/' + pc_path, 'r', encoding='utf-8') as g:
            amp_str = g.read()
        new_str = re.sub(r'<div id="site_main_data">.+?<div class="subdisc">', base_str + '<div class="subdisc">',
                         amp_str)
        with open('reibun/html_files/amp/sitepage/' + pc_path, 'w', encoding='utf-8') as h:
            h.write(new_str)


def make_amp_ranking():
    with open('reibun/html_files/pc/sitepage/ranking.html', 'r', encoding='utf-8') as f:
        pc_str = f.read()
    with open('reibun/html_files/amp/sitepage/ranking.html', 'r', encoding='utf-8') as g:
        amp_str = g.read()
    pc_str = file_upload.tab_and_line_feed_remove_from_str(pc_str)
    ...
    print(amp_str)
    with open('reibun/html_files/amp/sitepage/ranking.html', 'w', encoding='utf-8') as h:
        h.write(amp_str)


def pc_and_amp_site_page_upload(pd):
    pc_list = ['reibun/html_files/pc/sitepage/' + x for x in os.listdir('reibun/html_files/pc/sitepage') if '.html' in x and '_test'
               not in x and '_copy' not in x]
    amp_list = ['reibun/html_files/amp/sitepage/' + x for x in os.listdir('reibun/html_files/amp/sitepage') if '.html' in x and '_test'
                not in x and '_copy' not in x]
    ...
    if img_str_list:
        for img_str in img_str_list:
            if 'width="' in img_str and 'height="' in img_str and 'layout="' in img_str:
                ...
                img_path = img_path.replace('../images/', 'reibun/html_files/pc/images/')
...
def insert_data(site_data, site_user_data):
    ...
    for site_code in site_page_dict:
        ...
        if site_user_data[site_code]:
            ...
            site_page_path = 'reibun/html_files/pc/sitepage/' + site_page_dict[site_code] + '.html'
...
def insert_to_ranking(site_data):
    ...
    print(long_str)
    with open('reibun/html_files/pc/sitepage/ranking.html', 'w', encoding='utf-8') as g:
        g.write(long_str)
...
if __name__ == '__main__':
    ...
    # reibun_upload.tab_and_line_feed_remover('reibun/html_files/amp/site/index.html')
    # reibun_upload.tab_and_line_feed_remover('reibun/html_files/amp/sitepage/194964.html')

    # reibun_upload.tab_and_line_feed_remover('reibun/html_files/pc/sitepage_test/happymail.html')
    # reibun_upload.tab_and_line_feed_remover('reibun/html_files/pc/sitepage_test/194964.html')

    # site_page_pc_to_amp_changer('reibun/html_files/amp/sitepage/194964.html')

    # pc_and_amp_site_page_upload()

    # reibun_upload.ftp_upload(['reibun/html_files/pc/site/index.html'])

    main(reibun.main_info.info_dict)
    # insert_data_to_amp_site_page()
    # make_amp_ranking()

    # manual_add_modify_log(['reibun/html_files/pc/sitepage/{}.html'.format(x[1]) for x in site_str])
