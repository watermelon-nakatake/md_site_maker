...
def translate_md_to_html(temp_path, md_path_dir, sub_sex, pub_flag, pub_take_over, html_dir):
    if not os.path.exists('sfd/html_files/' + html_dir):
        os.mkdir('sfd/html_files/' + html_dir)
    ...
    for md_path in os.listdir(md_path_dir):
        ...
        if pub_take_over and os.path.exists('sfd/html_files/' + html_dir + '/' + md_path.replace('.md', '.html')):
            with open('sfd/html_files/up_data/' + md_path.replace('.md', '.html'), 'r', encoding='utf-8') as h:
                old_s = h.read()
        ...
        # print(temp)
        with open('sfd/html_files/' + html_dir + '/' + md_path.replace('.md', '.html'), 'w', encoding='utf-8') as g:
            g.write(temp)


if __name__ == '__main__':
    translate_md_to_html('sfd/wp_temp.html', 'sfd/md_files/sf_s', 'man', pub_flag=True, pub_take_over=True,
                         html_dir='up_data_s')
