...
side_bar_list = {'important': [],
                 'pop': []}
category_data = {'deaisite': ['ネットの出会い', 'index.html', 1],
                 'knowledge': ['出会いの基礎知識', 'index.html', 2],
                 'lover': ['恋人の作り方', 'index.html', 3],
                 'marriage': ['ネットで婚活', 'index.html', 4],
                 'men': ['ネットで彼女を作る', 'index.html', 5],
                 'policy': ['ポリシー', 'index.html', 6],
                 'women': ['ネットで彼氏を作る', 'index.html', 7]}
...
eyec_img = {'img_path': 'koibito_img.jpg', 'height': '800', 'width': '1200'}
aff_dir = {'dir': 'link'}
...
def koibito_icon_filter(long_str):
...
def koibito_search_category(directory, file_name):
    if directory != 'majime':
        category = directory
    else:
        if 'm0' in file_name:
...
def koibito_change_category_class(new_str, category):
    if category != 'majime':
...
def koibito_insert_additional_str(new_str):
...
def koibito_img_filter(new_str):
...
def koibito_breadcrumb_maker(category, directory, file_name):
    result = ''
