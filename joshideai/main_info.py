...
add_files = ['joshideai/html_files/index.html',
             'joshideai/html_files/css/main.css', 'joshideai/html_files/sitemap.xml']
...
sc_url = {'ワクワクメール': 'site_page/550909', 'PCMAX': 'site_page/pcmax',
          ...
          '出会い系口コミランキング': 'site/ranking.html', '出会い系口コミ評価ランキング': 'site/ranking.html',
          '出会い系サイト口コミランキング': 'site/ranking.html'}
...
def joshideai_insert_site_banner(long_str):
...
def joshideai_icon_filter(long_str):
...
def joshideai_search_category(directory, file_name):
    if directory != 'majime':
        category = directory
    else:
        if 'm0' in file_name:
...
def joshideai_change_category_class(new_str, category):
    if category != 'majime':
...
def joshideai_insert_additional_str(new_str):
...
def joshideai_img_filter(new_str):
...
def joshideai_breadcrumb_maker(category, directory, file_name):
    result = ''
